...
bool Pawn::makeMove(int * toPosition,Board * board){
	...
	if(this->isMoveForward(toPosition) && !board->occupied(toPosition)){
	...
	else if(this->isMoveTwoSteps(toPosition)){
...
bool Pawn::canPromote(){
	int * currentPosition = this->getPosition();
	if(this->getColor()==WHITE && currentPosition[0]==7)
		return TRUE;	
	else if(this->getColor()==BLACK && currentPosition[0]==0)
			return TRUE;
